...
import { User } from "../src/users/class.ts";
import { UserService } from "../src/users/service.ts";
import { DanetApplication } from "@danet/core";

let app: DanetApplication;
let server;
let todoService: UserService;
let port: number;
...
describe("Todo", () => {
    beforeAll(async () => {
        ...
        todoService = await app.get<UserService>(UserService);
        port = server.port;
    });
    ...
    it("simple todo creation", async () => {
        ...
        const returnedData: User = await res.json();
    ...
    it("get all todos", async () => {
        ...
        const plainArray = JSON.parse(
            JSON.stringify([firstAdded, secondAdded]),
        );
    ...
    it("get one todo by id", async () => {
        ...
        const res = await fetch(
            `http://localhost:${port}/todo/${firstAdded._id}`,
            {
                method: "GET",
            },
        );
        const returnedTodo: User = await res.json();
    ...
    it("update one todo by id", async () => {
        ...
        const returnedTodo = await (await fetch(
            `http://localhost:${port}/todo/${firstAdded._id}`,
            {
                method: "GET",
            },
